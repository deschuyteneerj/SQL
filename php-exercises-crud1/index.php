...
try {
    $bdd = new PDO('mysql:host=localhost;dbname=colyseum;charset=utf8', 'root', 'Password_1');
}
catch(Exception $e) {
...
$result = $bdd->query("SELECT * FROM clients LIMIT 0,20;");
...
$result = $bdd->query("SELECT * FROM clients WHERE card = 1;");
...
$result = $bdd->query("SELECT CONCAT('Nom: ', lastName, '<br />Prénom: ', firstName, '<br />Date de naissance: ', birthDate,IF(card <> 0,CONCAT('<br />Carte de fidélité: Oui<br />Numéro de carte: ',cardNumber), '<br />Carte de fidélité: Non')) AS Clients FROM clients;");
